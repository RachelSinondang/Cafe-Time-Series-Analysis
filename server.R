...
shinyServer(function(input, output) {
    ...
    output$plot_period <- renderPlotly({
        ...
        period_plot <- ggplot(rerata_sold %>% filter(Item == input$item1 | Item == input$item2 | Item == input$item3 | Item == input$item4), aes(x=weekday_weekend, y=mean, fill=period_day, text = glue("{period_day}
